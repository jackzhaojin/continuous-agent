/**
 * EDS harness orchestrator — native TypeScript port.
 *
 * Sibling of the generic orchestrator. The structure mirrors generic's
 * orchestrate() loop because both harnesses target the same
 * ai-docs/SPEC/{CONSTITUTION,WHY_WHAT,HOW,TASKS.json,STATUS.json} layout and
 * run the same 4-agent WHY/WHAT/HOW/WHEN spec pipeline followed by per-task
 * RESEARCH → BUILD → VALIDATE. EDS-specific deltas:
 *
 *   1. Runs ensureIgnoreFiles() before the spec pipeline so .gitignore and
 *      .hlxignore exclude ai-docs/ + .playwright-mcp/ from AEM EDS ingest.
 *   2. The prompts under ./prompts/ are EDS-specialized (block authoring,
 *      Playwright-first validation, da.live branch-and-push via Bash from
 *      inside the build agent).
 *   3. Build/validate agents always have Playwright allowed — EDS output is
 *      always a web target.
 *
 * Everything else (state store, subtask creation, handoff extraction,
 * resume/detect logic) is the same as generic. Consolidation is deferred to
 * v2.3 P6.
 *
 * Original deviations from the JS harness:
 *
...
import { ensureIgnoreFiles } from './ignore-files.js';
...
export function runEdsOrchestrator(config: HarnessRunConfig): AsyncIterable<HarnessEvent> {
...
async function orchestrate(config: HarnessRunConfig, bus: HarnessEventBus): Promise<void> {
  ...
  // ── EDS-specific: ensure .gitignore + .hlxignore exclude ai-docs / .playwright-mcp
  const ignoreUpdates = await ensureIgnoreFiles(codeDir);
  if (ignoreUpdates.gitignore) {
    await appendProgress(docsDir, '.gitignore updated to exclude .playwright-mcp');
  }
  if (ignoreUpdates.hlxignore) {
    await appendProgress(docsDir, '.hlxignore updated to exclude ai-docs/ and .playwright-mcp');
  }
  ...
  while (state.phase === 'EXECUTING') {
    ...
    if (validatePassed) {
      ...
      // See generic/orchestrator.ts for full rationale. Cap recursion depth.
...
async function runAgent(args: RunAgentArgs): Promise<HarnessAgentResult> {
  ...
  const agentConfig = EDS_AGENT_CONFIGS[agent];

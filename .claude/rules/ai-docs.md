...
7. **Prefix doc filenames with `YYYY-MM-DD-` (by initial creation) wherever it makes sense.** Inside a version folder, **sequence matters a lot** — these docs tell a story (vision → decision → build → run → fix), and a date prefix makes that order legible at a glance without opening git. Use the file's *initial* creation date (`git log --diff-filter=A --follow`), not the last-edited date. Prompt/session logs keep the `conversation-logger` format layered on top: `prompt-log-YYYY-MM-DD-{N}-{topic}.md` (N = zero-based within-day sequence). Pure data files (e.g. snapshot JSON) and the folder's own `README.md` index are exempt. If a doc is referenced by a stable path elsewhere (runtime code, another doc), renaming it means fixing those references too — do that in the same change.
